...
have a pair of queens, then highest cards in each hand are compared (see example 4 below); if the highest cards tie
...
func calc(args ...interface{}) (result string, err error) {
	...
	wins := 0
	for _, s := range textNumbers {
		hand := handToIntSlice(s)
		if wonByPlayerOne(hand) {
			wins++
		}
	}

	result = strconv.Itoa(wins)
	return
}

func wonByPlayerOne(hand []int) bool {
	ch1 := make(chan int)
	ch2 := make(chan int)

	go rankPlayerHand(hand[:5], ch1)
	go rankPlayerHand(hand[5:], ch2)

	plOneRank := <-ch1
	plTwoRank := <-ch2

	if plOneRank > plTwoRank {
		return true
	} else if plOneRank < plTwoRank {
		return false
	}

	// if ranks are equal, something went wrong
	panic(fmt.Sprintf("%d", plOneRank))
}
